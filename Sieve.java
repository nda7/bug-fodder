...
public class Sieve {
    ...
    //used for unit tests of printSieve. Will show up on find bugs as an error
    protected static String unitTest; 
    ...
    public static void printSieve(int[] results) {
    	...
    	//results will never equal null, always true. 
    	//if the length is 1, only prime num is 1 which we ignore. Print blank.
    	System.out.print("> ");
    	if (results.length != 1) {    
    		unitTest = "> ";
    ...
    public static int calculateMax(String[] args) {
		...
		if (args.length > 0) {
		    ...
		    throw new IllegalArgumentException();        //added throw clause here
		}
		return toReturn;
    }
